...
public class GameManager implements IGameManager {
    ...
    protected void resolveClearMinefield(Entity ent, Minefield mf) {
        ...
        if (environmentalEffectManager.clearMinefield(mf, ent, clear, boom, vPhaseReport, this)) {
            environmentalEffectManager.removeMinefield(mf, this);
        }
     ...
     */
    protected void triggerAPPod(Entity entity, int podId) {
        ...
        for (Entity target : game.getEntitiesVector(entity.getPosition())) {
            ...
            if (target.isConventionalInfantry()) {
                ...
                // Damage the platoon.
                reportManager.addReport(damageEntity(target, new HitData(Infantry.LOC_INFANTRY), damage), this);
     ...
     */
    protected void triggerBPod(Entity entity, int podId, Entity target) {
        // Get the mount for this pod.
        Mounted mount = entity.getEquipment(podId);
        ...
        if (target.isConventionalInfantry()) {
            ...
            // Damage the squad.
            reportManager.addReport(damageEntity(target, target.rollHitLocation(0, 0), damage), this);
            reportManager.addReport(damageEntity(target, target.rollHitLocation(0, 0), damage), this);
            reportManager.addReport(damageEntity(target, target.rollHitLocation(0, 0), damage), this);
            reportManager.addReport(damageEntity(target, target.rollHitLocation(0, 0), damage), this);
     ...
     */
    protected void resolveUnjam(Entity entity) {
        Report r;
    ...
    protected void resolveFindClub(Entity entity) {
        ...
        if (curHex.terrainLevel(Terrains.ARMS) > 0) {
            ...
            communicationManager.sendChangedHex(entity.getPosition(), this);
            r = new Report(3035);
            r.subject = entity.getId();
            r.addDesc(entity);
            addReport(r);
     ...
     */
    protected void resolvePunchAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (rollValue < toHit.getValue()) {
            ...
            if (targetInBuilding && (bldg != null)) {
                ...
                if (damage > 0) {
                    ...
                    reportManager.addReport(buildingReport, this);
                }
            }
        ...
        if ((target.getTargetType() == Targetable.TYPE_BUILDING)
            ...
            reportManager.addReport(damageInfantryIn(bldg, damage, target.getPosition()), this);
        ...
        if (damage == 0) {
            ...
            reportManager.addReport(damageEntity(te, hit, damage, false, damageType, false,
                    false, throughFront), this);
            if (target instanceof VTOL) {
                // destroy rotor
                reportManager.addReport(applyCriticalHit(te, VTOL.LOC_ROTOR,
            ...
            if (paa.isBladeExtended(paa.getArm())) {
                ...
                if (!(target instanceof Infantry)) {
                    addNewLines();
                    reportManager.addReport(criticalEntity(te, hit.getLocation(), hit.isRear(), 0,
                            true, false, damage), this);
                }
                ...
                if ((target instanceof BattleArmor) && (hit.getLocation() < te.locations())
                    ...
                    reportManager.addReport(damageEntity(te, baHit, 0), this);
                }
                // extend the blade
     ...
     */
    protected void resolveKickAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (damage == 0) {
            ...
            if (te.hasQuirk(OptionsConstants.QUIRK_NEG_WEAK_LEGS)) {
                addNewLines();
                reportManager.addReport(criticalEntity(te, hit.getLocation(), hit.isRear(), 0, 0), this);
            }
        }
     ...
     * Handle a kick attack
     */
    protected void resolveJumpJetAttack(PhysicalResult pr, int lastEntityId) {
     ...
     */
    protected void resolveProtoAttack(PhysicalResult pr, int lastEntityId) {
     ...
     */
    protected void resolveBrushOffAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (rollValue < toHit.getValue()) {
            ...
            r.newlines = 0;
            addReport(r);
            reportManager.addReport(damageEntity(ae, hit, damage), this);
            addNewLines();
        ...
        switch (target.getTargetType()) {
            case Targetable.TYPE_ENTITY:
                ...
                r.add(te.getLocationAbbr(hit));
                addReport(r);
                reportManager.addReport(damageEntity(te, hit, damage), this);
                addNewLines();
     ...
     */
    protected void resolveThrashAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (diceRoll2.getIntValue() < rollData.getValue()) {
            r.choose(false);
            addReport(r);
            reportManager.addReport(doEntityFall(ae, rollData), this);
        } else {
            r.choose(true);
            addReport(r);
        }
     ...
     * Handle a thrash attack
     */
    protected void resolveBAVibroClawAttack(PhysicalResult pr, int lastEntityId) {
     ...
     */
    protected void resolveClubAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (caa.getClub().getType().hasSubType(MiscType.S_LANCE)
            ...
            if (diceRoll2.getIntValue() >= 10) {
                ...
                reportManager.addReport(damageEntity(te, hit, 1, false, DamageType.NONE,
                        true, false, throughFront), this);
            }
        }
     ...
     */
    protected void resolvePushAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (Compute.isValidDisplacement(game, te.getId(), te.getPosition(), direction)) {
            ...
            reportManager.addReport(utilityManager.doEntityDisplacement(te, src, dest, pushPRD, this), this);
     ...
     */
    protected void resolveTripAttack(PhysicalResult pr, int lastEntityId) {
     ...
     */
    protected void resolveGrappleAttack(PhysicalResult pr, int lastEntityId) {
    ...
    protected void resolveGrappleAttack(PhysicalResult pr, int lastEntityId, int aeGrappleSide,
        ...
        if (aeGrappleSide == Entity.GRAPPLE_BOTH) {
            ...
            reportManager.addReport(utilityManager.doSetLocationsExposure(ae, game.getBoard().getHex(pos), false, ae.getElevation(), this), this);
        }
     ...
     */
    protected void resolveBreakGrappleAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (ae.isGrappleAttacker()) {
            ...
            reportManager.addReport(utilityManager.doEntityDisplacement(ae, ae.getPosition(), hexes[best], psr, this), this);
            ...
            psr.addModifier(TargetRoll.AUTOMATIC_SUCCESS, "break grapple");
            reportManager.addReport(utilityManager.doEntityDisplacement(te, te.getPosition(), hexes[worst], psr, this), this);
     ...
     */
    protected void resolveChargeAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (!target.getPosition().equals(caa.getTargetPos())) {
            ...
            r.subject = ae.getId();
            addReport(r);
            reportManager.addReport(utilityManager.doEntityDisplacement(ae, ae.getPosition(), caa.getTargetPos(), null, this), this);
            return;
        }
        ...
        if (rollValue < toHit.getValue()) {
            ...
            reportManager.addReport(utilityManager.doEntityDisplacement(ae, src, dest, null, this), this);
     ...
     */
    protected void resolveAirmechRamAttack(PhysicalResult pr, int lastEntityId) {
     ...
     */
    protected void resolveTeleMissileAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if (amsDamage > 0) {
            ...
            reportManager.addReport(damageEntity(ae, hit, amsDamage, false,
                    DamageType.NONE, false, false, false), this);
        ...
        if (rollValue < toHit.getValue()) {
            ...
            reportManager.addReport(damageEntity(te, hit,
                    ...
                    DamageType.NONE, false, false, throughFront), this);
            entityActionManager.destroyEntity(ae, "successful attack", this);
        }
     ...
     */
    protected void resolveRamAttack(PhysicalResult pr, int lastEntityId) {
    ...
    protected void resolveRamDamage(IAero aero, Entity te, ToHitData toHit, boolean glancing,
        ...
        if ((2 * ae.getOArmor(hit)) < damageTaken) {
            reportManager.addReport(entityActionManager.destroyEntity(ae, "by massive ramming damage", false, this), this);
        } else {
        ...
        if ((2 * te.getOArmor(hit)) < damage) {
            reportManager.addReport(entityActionManager.destroyEntity(te, "by massive ramming damage", false, this), this);
        } else {
            reportManager.addReport(damageEntity(te, hit, damage, false, DamageType.NONE,
                    false, false, throughFront), this);
        }
    ...
    protected void resolveChargeDamage(Entity ae, Entity te, ToHitData toHit, int direction,
        ...
        if (ae instanceof Tank) {
            ...
            reportManager.addReport(vehicleMotiveDamage((Tank) ae, mod), this);
        }
        ...
        while (damageTaken > 0) {
            ...
            reportManager.addReport(damageEntity(ae, hit, cluster, false, DamageType.NONE,
                    false, false, throughFront), this);
        }
        ...
        if ((te instanceof Tank) && !(te instanceof VTOL)) {
            ...
            reportManager.addReport(vehicleMotiveDamage((Tank) te, mod), this);
        }
        ...
        if (airmechRam) {
            if (!ae.isDoomed()) {
                ...
                if (diceRoll.getIntValue() < controlRoll.getValue()) {
                    r.choose(false);
                    reports.add(r);
                    entityActionManager.crashAirMech(ae, controlRoll, reports, this);
                } else {
                    r.choose(true);
                    reports.addElement(r);
                    if (ae instanceof LandAirMech) {
                        reports.addAll(entityActionManager.landAirMech((LandAirMech) ae, ae.getPosition(), 1, ae.delta_distance, this));
                    }
                }
                reportManager.addReport(reports, this);
            }
        } else {
            ...
            if (Compute.isValidDisplacement(game, te.getId(), te.getPosition(), direction)) {
                addNewLines();
                reportManager.addReport(utilityManager.doEntityDisplacement(te, src, dest, new PilotingRollData(
                        te.getId(), 2, "was charged"), this), this);
                reportManager.addReport(utilityManager.doEntityDisplacement(ae, ae.getPosition(), src, chargePSR, this), this);
            }
    ...
    protected void resolveLayExplosivesAttack(PhysicalResult pr) {
     ...
     */
    protected void resolveDfaAttack(PhysicalResult pr, int lastEntityId) {
        ...
        if ((target == null) || ((target.getTargetType() == Targetable.TYPE_ENTITY)
            ...
            if (ae.isProne()) {
                ...
                reportManager.addReport(doEntityFall(ae, daa.getTargetPos(), 2, 3,
                        ae.getBasePilotingRoll(), false, false), this);

            } else {
                // same effect as successful DFA
                ae.setElevation(ae.calcElevation(aeHex, teHex, 0, false, false));
                reportManager.addReport(utilityManager.doEntityDisplacement(ae, ae.getPosition(),
                        daa.getTargetPos(), new PilotingRollData(ae.getId(), 4,
                                "executed death from above"), this), this);
            }
            return;
        }
        ...
        if (!target.getPosition().equals(daa.getTargetPos())) {
            ...
            // entity isn't DFAing any more
            ae.setDisplacementAttack(null);
            reportManager.addReport(utilityManager.doEntityFallsInto(ae, ae.getElevation(), ae.getPosition(), daa.getTargetPos(),
                    ae.getBasePilotingRoll(), true, this), this);
            return;
        }
        ...
        if (rollValue < toHit.getValue()) {
            ...
            if (targetDest != null) {
                ...
                reportManager.addReport(utilityManager.doEntityDisplacement(te, dest, targetDest, null, this), this);
                ...
                // entity isn't DFAing any more
                ae.setDisplacementAttack(null);
                reportManager.addReport(doEntityFall(ae, dest, 2, 3, ae.getBasePilotingRoll(),
                        false, false), 1, this);
                ...
                if (violation != null) {
                    ...
                    vPhaseReport.addAll(utilityManager.doEntityDisplacement(violation, dest,
                            targetDest, new PilotingRollData(violation.getId(),
                                    0, "domino effect"), this));
                ...
                reportManager.addReport(entityActionManager.destroyEntity(ae, "impossible displacement",
                        ae instanceof Mech, ae instanceof Mech, this), this);
            }
            return;
        }
            ...
            if (target instanceof VTOL) {
                ...
                reportManager.addReport(applyCriticalHit(te, VTOL.LOC_ROTOR,
                        new CriticalSlot(CriticalSlot.TYPE_SYSTEM,
                                VTOL.CRIT_ROTOR_DESTROYED), false, 0, false), this);
            }
            // Target entities are pushed away or destroyed.
            Coords targetDest = Compute.getValidDisplacement(game, te.getId(), dest, direction);
            if (targetDest != null) {
                reportManager.addReport(utilityManager.doEntityDisplacement(te, dest, targetDest,
                        new PilotingRollData(te.getId(), 2, "hit by death from above"), this), this);
            } else {
                // ack! automatic death! Tanks
                // suffer an ammo/power plant hit.
                // TODO : a Mech suffers a Head Blown Off crit.
                reportManager.addReport(entityActionManager.destroyEntity(te, "impossible displacement",
                        te instanceof Mech, te instanceof Mech, this), this);
            }
        ...
        while (damageTaken > 0) {
            ...
            hit.setGeneralDamageType(HitData.DAMAGE_PHYSICAL);
            reportManager.addReport(damageEntity(ae, hit, cluster), this);
            damageTaken -= cluster;
        }

        if (ae.hasQuirk(OptionsConstants.QUIRK_NEG_WEAK_LEGS)) {
            addNewLines();
            reportManager.addReport(criticalEntity(ae, Mech.LOC_LLEG, false, 0, 0), this);
            addNewLines();
            reportManager.addReport(criticalEntity(ae, Mech.LOC_RLEG, false, 0, 0), this);
            if (ae instanceof QuadMech) {
                addNewLines();
                reportManager.addReport(criticalEntity(ae, Mech.LOC_LARM, false, 0, 0), this);
                addNewLines();
                reportManager.addReport(criticalEntity(ae, Mech.LOC_RARM, false, 0, 0), this);
            }
        }
        ...
        reportManager.addReport(utilityManager.doEntityDisplacement(ae, dest, dest, new PilotingRollData(
                ae.getId(), 4, "executed death from above"), this), this);
     ...
     */
    protected void resolvePhysicalAttack(PhysicalResult pr, int cen) {
